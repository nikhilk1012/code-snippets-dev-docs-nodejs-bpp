...
const search = async ({ headers, body }, res) => {
  try {
    ...
    const providers = await deliveryService.findDeliveryProviders(startLoc, endLoc);
    // ... Getting BPP Description
    const description = await deliveryService.findDeliveryDesc();
    let message = {
      catalog: {
    ...
    await util.respond(headers, context, message, "/delivery/on_search");
  } catch (error) {
 ...
 * @param {object} req Api request object.
 * @param {object} res Api response object.
 * @return {object} returns the quotes provided by deliveryService
 */
const select = async ({ headers, body }, res) => {
  try {
    ...
    res.status(200).send(util.httpResponse("ACK"));
    let message = { order: {} };
    const { quote, provider, items } = deliveryService.returnQuoteOnSelectedItems(itemIds);
    message.order = { provider, items, quote };
    await util.respond(headers, context, message, "/delivery/on_select");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const init = async ({ headers, body }, res) => {
  try {
    ...
    const isvalidated = deliveryService.validateOrderOnDetails(order);
    if (isvalidated) {
      order.quote = deliveryService.getQuote(order);
      order.payments = deliveryService.getPaymentDetails(order.quote);
    }
    await deliveryService.saveOrder(order);
    const message = {
      order,
    };
    await util.respond(headers, context, message, "/delivery/on_init");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const confirm = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const isvalidated = deliveryService.validateOrderOnPayment(order, paymentTransactionId);
    if (isvalidated) {
      const message = {
        order,
      };
      await util.respond(headers, context, message, "/delivery/on_confirm");
    } else {
...
const cancel = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const cancelledOrder = await deliveryService.cancelOrder(orderId);
    let message = {
      order: cancelledOrder,
    };
    await util.respond(headers, context, message, "/delivery/on_cancel");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const update = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const updatedOrder = await deliveryService.saveOrder(order);
    let message = {
      order: updatedOrder,
    };
    await util.respond(headers, context, message, "/delivery/on_update");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const track = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const trackingDetails = deliveryService.getTrackingDetails(orderId);
    let message = {
      tracking: trackingDetails,
    };
    await util.respond(headers, context, message, "/delivery/on_track");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const rate = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const feedback = await deliveryService.rateOrder(orderId, value);
    let message = {
      feedback,
    };
    await util.respond(headers, context, message, "/delivery/on_rating");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
...
const support = async ({ headers, body }, res) => {
  try {
    ...
    // ... Returns the ack immediately and continue the processing after validation
    res.status(200).send(util.httpResponse("ACK"));
    const supportDetails = await deliveryService.getSupportDetails(orderId);
    let message = {
      ...supportDetails,
    };
    await util.respond(headers, context, message, "/delivery/on_support");
  } catch (error) {
    res.status(500).send(util.httpResponse("NACK", error));
  }
};
